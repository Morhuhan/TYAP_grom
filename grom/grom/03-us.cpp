...
// Нетерменал бесполезен в граматике, если при достяжении каким-то образом этого нетерминала, мы уже никогда не получим терминальную цепочку.
// В данном случае <A>=<A><B> бесполезен, так как мы получим <A>[b][b][b][b]....... <A> уже не пропадет и цепочка не станет терминальной.
// B тоже бесполезен, так как его не получить без участия A